...
public class AssignmentRoleUseCaseImpl implements AssignmentRoleUseCase {
    ...
    public AssignmentRoleUseCaseImpl(
            UserAvalonRepositoryPort userPort,
            RoleAssignmentRepositoryPort rolePort,
    ...
    public AssignmentRoleResponse execute(AssignmentRoleRequestDto request) {
        ...
        MasterRoot userStatus = tree.getByIdOrThrow(user.getStatusId());

        MasterRoot role;
        if (request.roleId() != null) {
            role = tree.getByIdOrThrow(request.roleId());
        } else if (request.roleCode() != null && !request.roleCode().isBlank()) {
            role = tree.getByCode(request.roleCode().trim().toUpperCase());
            if (role == null) {
                throw new EntityNotFoundException("no se pudo encontrar el rol para asignar con codigo: " + request.roleCode());
            }
        } else {
            throw new EntityNotFoundException("no se pudo encontrar el rol para asignar");
        }
        ...
        MasterRoot statusActive = tree.getByCodeOrThrow("ACT");
